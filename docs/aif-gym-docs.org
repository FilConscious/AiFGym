...
| ~--index_Si~      | ~-i~      | Index @@latex:$i$@@ for selecting a random variable  @@latex:$S_{i}$@@                                      | ~0~ (default)                 |
...
| ~--state_A~       | ~-sa~     | Index $i$ for selecting a $Q(O_{i} = o_{j}\vert s)$ (a column of matrix $\mathbf{A}$) to plot             | ~0~ (default)                 |
